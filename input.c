...
static bool
server_handle_shortcut(struct wl_listener *listener,
		       struct wlr_keyboard_key_event *event)
{
	...
	int symsc =
	    xkb_state_key_get_syms(keyboard->wlr_keyboard->xkb_state,
				   keycode, &symsv);
	...
	if(event->state == WL_KEYBOARD_KEY_STATE_PRESSED){
		for(int i = 0; i < symsc; i++){
			unsigned int vt =
			    symsv[i] - XKB_KEY_XF86Switch_VT_1 + 1;
			if(vt >= 1 && vt <= 12){
				change_vt(server, vt);
...
static void
server_new_keyboard(struct wio_server *server,
		    struct wlr_input_device *device)
{
	...
	struct wio_keyboard *keyboard =
	    calloc(1, sizeof(struct wio_keyboard));
...
static void
server_new_pointer(struct wio_server *server,
		   struct wlr_input_device *device)
{
		   ...
		   struct wlr_pointer_button_event *event)
{
	...
	case 5:{
			...
			server->menu.restore_active_textures =
			    calloc(count, sizeof(struct wlr_texture *));
			server->menu.restore_inactive_textures =
			    calloc(count, sizeof(struct wlr_texture *));
...
static void
view_begin_interactive(struct wio_view *view, struct wlr_surface *surface,
		       double sx, double sy, const char *cursor,
		       enum wio_input_state state)
{
...
new_view(struct wio_server *server)
{
	...
	if(box.width < MINWIDTH || box.height < MINHEIGHT){
		wlr_log(WLR_ERROR,
			"new_view: boxtoo small (%dx%d), need >= %dx%d",
	...
	char cmd[1024];
	if(snprintf
	   (cmd, sizeof(cmd), "%s -- %s", server->cage,
	    server->term) >= (int) sizeof(cmd)){
...
static void
handle_button_internal(struct wio_server *server,
		       struct wlr_pointer_button_event *event)
{
