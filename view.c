...
xdg_toplevel_commit(struct wl_listener *listener, void *data)
{
	...
	struct wl_client *client =
	    wl_resource_get_client(view->xdg_toplevel->resource);
...
server_xdg_shell_new_toplevel(struct wl_listener *listener, void *data)
{
	struct wio_server *server =
	    wl_container_of(listener, server, xdg_shell_new_toplevel);
	...
	wl_signal_add(&xdg_toplevel->base->surface->events.destroy,
		      &view->destroy);
}

static void
xdg_toplevel_decoration_request_mode(struct wl_listener *listener,
				     void *data)
{
...
static void
xdg_toplevel_decoration_destroy(struct wl_listener *listener, void *data)
{
	struct wlr_xdg_toplevel_decoration_v1 *decoration = data;
...
void
server_new_toplevel_decoration(struct wl_listener *listener, void *data)
{
	...
	wl_signal_add(&wlr_xdg_toplevel_decoration->events.request_mode,
		      &d->request_mode);
	d->destroy.notify = xdg_toplevel_decoration_destroy;
	wl_signal_add(&wlr_xdg_toplevel_decoration->events.destroy,
		      &d->destroy);

	// force server-side decorations without waiting for the client to request the decoration mode
	xdg_toplevel_decoration_request_mode(&d->request_mode,
					     wlr_xdg_toplevel_decoration);
}

void
wio_view_focus(struct wio_view *view, struct wlr_surface *surface)
{
	if(view == NULL){
		return;
	}
...
void
wio_view_hide(struct wio_view *view)
{
	struct wio_server *server = view->server;
	struct wlr_seat *seat = server->seat;
...
void
wio_view_restore(struct wio_view *view)
{
	struct wio_server *server = view->server;
	...
	double *sx, double *sy)
{
	...
	_surface =
	    wlr_xdg_surface_surface_at(view->xdg_toplevel->base, view_sx,
				       view_sy, &_sx, &_sy);
...
static int
which_corner(struct wlr_box *box, int x, int y)
{
...
struct wio_view *
wio_view_at(struct wio_server *server, double lx, double ly,
...
void
wio_view_move(struct wio_view *view, int x, int y)
{
	view->x = x;
	view->y = y;
	...
	wl_list_for_each(output, &view->server->outputs, link){
		wlr_surface_send_enter(view->xdg_toplevel->base->surface,
				       output->wlr_output);
	}
}

struct wlr_box
wio_which_box(struct wio_server *server)
{
	struct wlr_box box;
	int x1 = 0;
	int x2 = 0;
...
struct wlr_box
wio_canon_box(struct wio_server *server, struct wlr_box box)
{
