...
struct wlr_texture *
render_menu_text(struct wlr_renderer *renderer,
		 cairo_t *cairo, cairo_surface_t *surf, const char *text)
{
	...
	if(extents.width > MENU_TEXT_MAX_WIDTH){
		...
		while(len > 0){
			len--;
			snprintf(buf, sizeof(buf), "%.*s...", (int) len,
				 text);
			cairo_text_extents(cairo, buf, &extents);
			if(extents.width <= MENU_TEXT_MAX_WIDTH){
				break;
	...
	return wlr_texture_from_pixels(renderer,
				       ...
				       (surf), extents.width + 2,
				       extents.height + 2, data);
}
...
static void
gen_menu_textures(struct wio_server *server)
{
	struct wlr_renderer *renderer = server->renderer;
	// numbers pulled from ass
	server->menu_text_surface =
	...
	char *text[] =
	    { "New", "Resize", "Move", "Delete", "Hide", "Restore" };
	for(size_t i = 0; i < countof(text); ++i){
		server->menu.inactive_textures[i] =
		    render_menu_text(renderer, cairo, surf, text[i]);
	}

	cairo_set_source_rgb(cairo, 1, 1, 1);
	for(size_t i = 0; i < countof(text); ++i){
		server->menu.active_textures[i] =
		    render_menu_text(renderer, cairo, surf, text[i]);
	}
}

static enum wl_output_transform
str_to_transform(const char *str)
{
...
void
parse_args(int argc, char *argv[], struct wio_server *server)
{
	int c;
	while((c = getopt(argc, argv, "c:t:o:h")) != -1){
		...
		case 'o':;
			...
			wl_list_insert(&server->output_configs,
				       &config->link);
		...
		case 'h':
			printf
			    ("Usage: %s [-t <term>] [-c <cage>] [-o <output config>...]\n",
			     argv[0]);
			exit(0);
		default:
...
static void
wio_inner_main(void *closure, int argc, char **argv)
{
	...
	server.backend =
	    wlr_backend_autocreate(wl_display_get_event_loop
	...
	wlr_compositor_create(server.wl_display, compositor_version,
			      server.renderer);
	...
	wl_list_for_each(config, &server.output_configs, link){
		if(config->scale > 1)
			wlr_xcursor_manager_load(server.cursor_mgr,
						 config->scale);
	}
	...
	server.xdg_shell_new_toplevel.notify =
	    server_xdg_shell_new_toplevel;
	wl_signal_add(&server.xdg_shell->events.new_toplevel,
		      &server.xdg_shell_new_toplevel);
