	      ...
	      struct wlr_box *usable_area, bool exclusive)
{
	...
	wl_list_for_each_reverse(wio_surface, list, link){
		...
		apply_exclusive(usable_area, state->anchor,
				...
				state->margin.left);
		wlr_layer_surface_v1_configure(layer, box.width,
					       box.height);
		if(layer->initialized){
			wlr_layer_surface_v1_configure(layer, box.width,
						       box.height);
...
void
arrange_layers(struct wio_output *output)
{
	...
	for(size_t i = 0; i < nlayers; ++i){
		wl_list_for_each_reverse(layer,
					 &output->
					 layers[layers_above_shell[i]],
					 link){
			if(layer->layer_surface->current.
...
handle_destroy(struct wl_listener *listener, void *data)
{
	...
	if(layer->layer_surface->output){
		wl_list_remove(&layer->output_destroy.link);
		arrange_layers((struct wio_output *) layer->layer_surface->
			       output->data);
	}
	free(layer);
}
...
handle_map(struct wl_listener *listener, void *data)
{
	...
	struct wlr_layer_surface_v1 *layer_surface = layer->layer_surface;
	wlr_surface_send_enter(layer_surface->surface,
